...
#### `channelGateway.telegramStatus` <span class="badge badge--info">query</span>

Check Telegram bot configuration status for this workspace. Returns whether a bot token is configured and the bot's username.
...
> Router key: `proactive`

#### `proactive.getPrefs` <span class="badge badge--info">query</span>

Get proactive AI preferences for the current workspace. Returns defaults if no preferences have been set.
...
#### `users.getFeedPreferences` <span class="badge badge--info">query</span>

Relay feed preferences (legacy client contract). Stored under user_preferences.ui_preferences.feedPreferences.
...
#### `workspaces.listMyInvites` <span class="badge badge--info">query</span>
...
*Generated 2026-04-20 from `synap-backend/packages/api/src/routers/`*
